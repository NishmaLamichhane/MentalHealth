...
<style>
  ...
  .form-page { min-height:100vh; background:linear-gradient(135deg,#e9f5f5 0%,var(--soft-white) 100%); display:flex; align-items:center; justify-content:center; padding:2.5rem 1.25rem; }
  ...
  .form-icon { width:52px; height:52px; background:var(--teal-light); border-radius:14px; display:flex; align-items:center; justify-content:center; margin-bottom:1.25rem; }
  .form-icon svg { width:26px; height:26px; color:var(--teal); }
  ...
  input:focus, textarea:focus { border-color:var(--teal); box-shadow:0 0 0 3px rgba(15,124,124,0.12); background:#fff; }
  textarea { resize:vertical; min-height:100px; }

  .submit-btn { width:100%; background:linear-gradient(135deg,var(--teal-mid),var(--teal)); color:#fff; padding:0.9rem; border:none; border-radius:12px; font-size:1rem; font-weight:700; font-family:'DM Sans',sans-serif; cursor:pointer; box-shadow:0 6px 20px rgba(15,124,124,0.35); transition:transform 0.2s, box-shadow 0.2s; margin-top:0.5rem; }
...
<div class="form-page">
  <div class="form-card">
    ...
    <h1 class="form-title">Add Progress Entry</h1>
    <p class="form-subtitle">Record your mental health milestones and reflections.</p>

    <form action="{{ route('user_progress.store') }}" method="POST">
      @csrf
      <div class="form-group">
        <label for="progress_title">Title</label>
        <input type="text" name="progress_title" id="progress_title" placeholder="e.g. Completed breathing exercise" required>
      </div>
      <div class="form-group">
        <label for="description">Description</label>
        <textarea name="description" id="description" placeholder="How did this session go? What did you feel?"></textarea>
      </div>
      <div class="form-group">
        <label for="progress_date">Date</label>
        <input type="date" name="progress_date" id="progress_date" required>
      </div>
