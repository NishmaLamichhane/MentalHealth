...
<h1 class="text-black font-bold text-3xl pt-3 dark:text-white"><i class="ri-psychotherapy-fill pr-2 text-2xl"></i>Therapist</h2>
    ...
    <table class="w-full mt-5">
        <tr class="bg-black text-white">
            ...
            <!-- <th class="border p-2 bg-black text-white">Time-Slots</th> -->
        ...
        <tr class="text-center dark:text-black bg-gray-400 ">
            ...
            <td class="border p-2 text-left">
                @if($therapist->available_time_slots)
                <ul class="list-disc pl-4">
                    @foreach(explode(',', $therapist->available_time_slots) as $slot)
                    <li>{{ \Carbon\Carbon::createFromFormat('H:i', $slot)->format('g:i A') }}</li>
                    @endforeach
                </ul>
                @else
                <span class="text-gray-500 italic">No time slots</span>
                @endif
            </td>
